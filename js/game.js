...
class AudioKit {
    ...
    initAudio(foods) {
        ...
        for (const [key, src] of Object.entries(sfx))
            this.createClip('sfx-' + key, src);
        for (const f of foods)
            this.createClip('voice-' + f.id, f.audio);
    }
    createClip(key, src) {
    ...
    tone(freq, delay, dur, type = 'sine', vol = 0.18) {
        if (!this.ctx)
            return;
...
class Game {
    ...
    init() {
        ...
        this.overlayCardEl = document.getElementById('overlayCard');
        this.speakerEl = document.getElementById('speaker');
        this.promptTextEl = document.getElementById('promptText');
        this.timerFillEl = document.getElementById('timerFill');
        this.roundBadgeEl = document.getElementById('roundBadge');
        this.playerScoreEl = document.getElementById('playerScore');
        this.robotScoreEl = document.getElementById('robotScore');
        this.playerAvatarEl = document.getElementById('playerAvatar');
        this.robotAvatarEl = document.getElementById('robotAvatar');
    ...
    onTap(cell) {
        if (this.phase !== 'playing' || !this.roundActive || !this.target)
            return;
        if (cell.taken)
            return;
    ...
    resolveRound(reason, tapped) {
        if (!this.roundActive || !this.target)
            return;
        this.roundActive = false;
        cancelAnimationFrame(this.rafId);
        this.audio.stopVoice();
    ...
    buildGrid() {
        ...
        for (const food of order) {
            ...
            const entry = { food, el, taken: false };
    ...
    flyFood(imgSrc, fromEl, toEl, onLand) {
    ...
    showCountdown(text, go) {
        this.showOverlay(`<div class="count-num ${go ? 'go' : ''}">${text}</div>` +
            `<div class="count-tip">${go ? '快抢！' : '准备好了吗？'}</div>`);
    }
    showResult(win, draw) {
        const title = draw ? '平局啦！' : win ? '你赢了！' : '你输了！';
        this.showOverlay(`<div class="result-panel">` +
